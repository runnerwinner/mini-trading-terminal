...
import {
  TradePanelProvider,
  useTradePanel,
} from "@/contexts/trade-panel-context";
import {
  Card,
  CardContent,
  CardHeader,
...
const TokenChart = lazy(() =>
  import("@/components/TokenChart").then((module) => ({
    default: module.TokenChart,
  })),
);
const TradingPanel = lazy(() =>
  import("@/components/TradingPanel").then((module) => ({
    default: module.TradingPanel,
  })),
);
const FloatingTradePanel = lazy(() =>
  import("@/components/FloatingTradePanel").then((module) => ({
    default: module.FloatingTradePanel,
  })),
);
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (
          ...
          eventsResult.value.getTokenEvents?.items
        ) {
          const tokenEvents = eventsResult.value.getTokenEvents.items
            ...
            .map((ev, index) => {
              ...
              const calculatedAmountUsd =
                swapValue * Math.abs(amount0 / 10 ** decimals);
        ...
        if (
          ...
          setPairs(
            pairsResult.value.filterPairs.results.filter(
              (pair) => pair != null,
            ) as PairFilterResult[],
          );
        }
            ...
            {details && (
              <Suspense
                fallback={
                  <Card>
                    <CardHeader>
                      <CardTitle>Trade {details.symbol || "Token"}</CardTitle>
                    </CardHeader>
                    <CardContent>
...
const FloatingTradePanelHost = memo(function FloatingTradePanelHost({
  token,
}: {
  token: EnhancedToken;
}) {
  ...
  return (
    <Suspense fallback={null}>
      <FloatingTradePanel token={token} />
    </Suspense>
  );
});
